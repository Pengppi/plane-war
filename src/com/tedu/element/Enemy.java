...
public class Enemy extends ElementObj{
	...
	private boolean haveAdd=false;//防止死亡后重复加分
	...
	public ElementObj createElement(String kind_str) {
		...
		{
		case "1"://普通敌机(单发)
		this.kindToEnemy(60, 60, 3, 0, 1,1);
		break;
		case "2"://双发敌机(双发)
		this.kindToEnemy(60, 60, 5, 0, 1,2);
		break;
		case "3"://巨型敌机(散射)
		this.kindToEnemy(90, 90, 12, 0, 1,3);
		break;
		case "4"://疾速敌机(单发)
		this.kindToEnemy(60, 60, 3, 0, 2,3);
		break;
		case "5"://小型敌机(向左，向右随机)
		this.kindToEnemy(40, 40, 2, new Random().nextBoolean()?1:-1, 1,4);
		break;
		case "6"://分裂者(中间停下，向四个方向发射)
		this.kindToEnemy(60, 60, 4, 0, 1,5);
		//设置分裂的时间间隔
		...
		setInterval(80);
		case "8"://激光敌机（发射激光）
		this.kindToEnemy(60, 60, 6, 0, 1,6);
		break;
		case "9"://自爆敌机（一定几率自爆）
		this.kindToEnemy(60, 60, 4, 0, 4,6);
		break;
		case "0"://导弹敌机（发射导弹）
		this.kindToEnemy(70, 70, 8, 0, 1,6);
		break;
		case "a":
		setInterval(80);
		case "c"://双重机枪精英敌机,双重激光精英敌机
		this.kindToEnemy(80, 80, 10, 0, 1,7);
		break;
		case "b"://三重重机枪敌机
		setInterval(100);
		this.kindToEnemy(80, 80, 12, 0, 1,7);
		break;
		case "e"://等离子敌机
		setInterval(300);
		case "d"://双重导弹敌机
	...
	//敌机种类初始化敌机属性函数(敌机大小,血量，运动方式(水平速度，垂直速度)，得分)
		public void kindToEnemy(int width,int height,int density,int moveXNum,int moveYNum,int star)
		{
			this.setW(width);
			this.setH(height);
	...
	@Override
	public int dieStar(){   //敌人死亡后加分
		if((!this.haveAdd)&&this.addStar){
			this.haveAdd=true;
			return this.star;
		}else return  0;
	}
