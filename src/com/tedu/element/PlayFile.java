    ...
    public ElementObj createElement(String str) {//子弹的初始化
        ...
        for (String str1 : split) {//X:3
            ...
            switch (split2[0]) {
                //子弹的坐标
                case "x":
                    this.setX(Double.parseDouble(split2[1]));
                    break;
                case "y":
                    this.setY(Double.parseDouble(split2[1]));
                    break;
                //子弹的水平与垂直速度
                case "hv":
                    this.setSpeed(Double.parseDouble(split2[1]), this.getYSpeed());
                    break;
                case "vv":
                    this.setSpeed(this.getXSpeed(), Double.parseDouble(split2[1]));
                    break;
                //子弹的阵营属性(地方还是我方)
                case "c":
                    this.setCamp(Integer.parseInt(split2[1]));
                    break;
                //子弹的种类(1 普通子弹,2 导弹,3 激光,4 等离子球)kind
                case "k":
                    this.setKind(split2[1]);
                    break;
            }
        }
